...
export async function setEpisodeVoiceOverride(feedId: number, episodeKey: string, voiceId: string | null) {
    ...
    try {
        await unlink(episodePath(feed.podcastSlug, resolveEpisodeKey(article.episodeKey, article.title, article.sourceUrl)))
    }
    catch {
    }
...
async function runEpisodeGenerationJob(job: EpisodeGenerationJob) {
    try {
        ...
        let outputPath = episodePath(feed.podcastSlug, resolveEpisodeKey(article.episodeKey, article.title, article.sourceUrl))
        ...
        try {
            await persistAudioStream(generated.audio.stream, outputPath, article, feed, job)
        } finally {
...
function buildAudioUrl(feed: Pick<Feed, 'podcastSlug'>, article: Pick<Article, 'episodeKey' | 'title' | 'sourceUrl'>) {
